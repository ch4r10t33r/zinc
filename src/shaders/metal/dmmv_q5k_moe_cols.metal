...
// grid.y is the real expert id, grid.z selects up to four packed route ids for
...
kernel void main0(
    ...
    for (uint b = 0u; b < blocks_per_row; b++) {
        ...
        for (uint g = 0u; g < 4u; g++) {
            ...
            if (active0) {
                acc.x += v_lo * x0[col_lo] + v_hi * x0[col_hi];
            }
            if (active1) {
                acc.y += v_lo * x1[col_lo] + v_hi * x1[col_hi];
            }
            if (active2) {
                acc.z += v_lo * x2[col_lo] + v_hi * x2[col_hi];
            }
            if (active3) {
                acc.w += v_lo * x3[col_lo] + v_hi * x3[col_hi];
            }
        }
    }
