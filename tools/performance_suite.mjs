...
async function runMetalTarget(args) {
  ...
  return {
    ...
    methodology: {
      runner: "ZINC CLI with same-host llama.cpp baselines",
      benchmark_style: "Local Apple Silicon scenario matrix",
      notes: [
        "Measurements are collected on the local Apple Silicon machine. ZINC and llama.cpp use the same GGUF file for each model.",
        "ZINC is built in ReleaseFast mode from the current workspace.",
        "Each model is evaluated across four scenarios: short prompt, medium context, long context, and extended decode.",
        "Reported values are medians from the measured runs after the warmup pass.",
        "Managed catalog models are resolved from the ZINC model cache when available.",
        "llama.cpp is measured through a per-model llama-server when available, reused across the scenario matrix; otherwise the suite records llama-cli results.",
        "Published runs assume no competing local workload is saturating CPU, memory bandwidth, or GPU.",
      ],
      runs: args.runs,
...
async function runRdnaTarget(args) {
  ...
  return {
    ...
    methodology: {
      runner: "ZINC CLI with same-host llama.cpp baselines",
      benchmark_style: "SSH-orchestrated scenario matrix on the RDNA node",
      notes: [
        "Measurements are collected on a single AMD Radeon AI PRO R9700 node. ZINC and llama.cpp use the same GGUF file for each model.",
        "ZINC is built in ReleaseFast mode and run with RADV cooperative matrix support enabled.",
        "Each model is evaluated across four scenarios: short prompt, medium context, long context, and extended decode.",
        "Reported values are medians from the measured runs after the warmup pass.",
        "llama.cpp is measured through a per-model llama-server when available, reused across the scenario matrix; otherwise the suite records llama-cli results.",
        "Published runs assume the benchmark node is dedicated to the test, with no competing GPU workload.",
      ],
      runs: args.runs,
      warmup_runs: args.warmupRuns,
    },
