...
//   Each simdgroup processes 2 rows => 8 rows per threadgroup.
// This keeps the same per-row llama.cpp matvec math while halving the
// LM-head partial count and threadgroup scheduling work for Gemma 31B.
//
...
kernel void main0(
    ...
    threadgroup float tg_best_vals[NSG * NR0];
    threadgroup uint tg_best_idxs[NSG * NR0];
    ...
    if (sgitg == 0 && tiisg == 0) {
        float best_val = tg_best_vals[0];
        uint best_idx = tg_best_idxs[0];
        FOR_UNROLL (uint i = 1; i < NSG * NR0; ++i) {
            const float v = tg_best_vals[i];
            const uint idx = tg_best_idxs[i];
            if (v > best_val || (v == best_val && idx < best_idx)) {
                best_val = v;
                best_idx = idx;
            }
        }
        partials[tgpig.x * 2u + 0u] = best_idx;
        partials[tgpig.x * 2u + 1u] = as_type<uint>(best_val);
    }
}
