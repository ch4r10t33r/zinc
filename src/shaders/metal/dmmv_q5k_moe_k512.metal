...
// Same one-simdgroup-per-row layout as dmmv_q5k_moe_k2048, with the two
// Q5_K blocks unrolled so the hot expert-down path does not pay the generic
// K<=2048 loop overhead.
...
inline void accumulate_q5k_block(
...
kernel void main0(
    ...
    const uint row = tg_pos.x * ROWS_PER_TG + simdgroup;
    if (row >= p.M) return;
    ...
    device const uchar* row_ptr = W + expert_base + ulong(row) * 352ul;

    float sum = 0.0f;
    accumulate_q5k_block(row_ptr, input, 0u, lane, sum);
    accumulate_q5k_block(row_ptr + 176u, input, 256u, lane, sum);
