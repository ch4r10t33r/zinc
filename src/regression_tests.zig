...
test "Metal prefillBatched validate path diffs last-token logits within 1e-3" {
    const src = @embedFile("compute/forward_metal.zig");
    try expectContainsNear(src, "pub fn prefillBatched(self: *InferenceEngine, state: *DecodeState, prompt_tokens: []const u32) !void {", "if (mode == .validate)", 18000);
...
test "Metal prefillBatched supports prefix reuse by extending KV at state.position" {
    ...
    try expectContainsNear(src, "pub fn prefillBatched(self: *InferenceEngine, state: *DecodeState, prompt_tokens: []const u32) !void {", "self.position = position_base + n_tokens;", 18000);
}
